...
// Variable shared between sim-main and sim-with-cuda
extern struct transit_center *transit_centers; // Array of transit centers accessible in the rank
extern int num_transit_centers;
extern struct delivery *deliveries; // Array of deliveries managed by the rank
extern int num_deliveries;
extern struct delivery **outgoing_deliveries; // The deliveries going to be sent to other MPI ranks
extern struct delivery *incoming_deliveries; // The deliveries going to be received from other MPI ranks
extern int world_rank; // Rank number of current MPI rank
extern int world_size; // Total number of MPI ranks

/***********************************************************************************************************/
// Input: None
// Output: None
// Purpose: Set up CUDA environment and memory allocation for current rank
/***********************************************************************************************************/
...
/***********************************************************************************************************/
// Input: delivery array of deliveries, tansit_center array of transit centers, integer number of deliveries
//        integer number of transit centers, delivery array of incoming deliveries, delivery array of outgoing deliveries
//        integer array of outgoing delivery count, interger number of total rank number, integer number of timestep
// Output: None
// Purpose: Perform simulation of one step of discrete time simulation on CUDA GPU (actual logic)
/***********************************************************************************************************/
__global__ void sim_kernel(struct delivery * deliveries, struct transit_center * transit_centers, unsigned int num_deliveries, 
	unsigned int num_transit_centers, struct delivery * incoming_deliveries, struct delivery ** outgoing_deliveries, int * outgoing_deliveries_count, 
	int world_size, unsigned int current_time) {
	
    // kernel function, simulates one unit of time
	for (int index = blockIdx.x * blockDim.x + threadIdx.x; index < num_deliveries; index += blockDim.x * gridDim.x) {
		// loop over deliveries, compute their next status
		if (deliveries[index].status == LOST_DELIVERY) {
			// check if can add new deliveries
			int recv_index = atomicSub(&outgoing_deliveries_count[world_size], 1) - 1;
			if (recv_index >= 0) memcpy(&deliveries[index], &incoming_deliveries[recv_index], sizeof(struct delivery));
		}
		...
		if (delivery_status == STAMPED) {
			...
			if (deliveries[index].transit_time == 0) {
				...
				int send_index = atomicAdd(&outgoing_deliveries_count[dest_rank], 1);
				if (current_rank != dest_rank) {
					memcpy(&outgoing_deliveries[dest_rank][send_index], &deliveries[index], sizeof(struct delivery));
...
/***********************************************************************************************************/
// Input: integer array of the number of deliveries sent to other ranks, integer of current time step, 
//        integer on hybrid or not
// Output: boolean
// Purpose: Perform simulation of one step of discrete time simulation on CUDA GPU (wrapper)
/***********************************************************************************************************/
extern "C" bool sim_kernelLaunch(int * outgoing_deliveries_count, unsigned int current_time, int hybrid) {
	if (hybrid == 1) { // MPI & CUDA, parallel code
